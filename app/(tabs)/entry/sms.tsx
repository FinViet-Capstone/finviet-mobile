...
export default function SMSEntryScreen() {
  ...
  const handleExtract = () => {
    ...
    extract.mutate(trimmed, {
      onSuccess: (result) => {
        ...
        setAmountUncertain(result.confidence.amount < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD);
        setMerchantUncertain(result.confidence.merchant < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD);
        setCategoryUncertain(result.confidence.categoryId < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD);
        setPhase('review');
      },
      onError: () => {
        Alert.alert('Không trích xuất được', 'Không thể tự động đọc tin nhắn.', [
          { text: 'Nhập tay', onPress: () => router.replace({ pathname: '/(tabs)/entry/manual', params: dateParam ? { date: dateParam } : undefined }) },
  ...
  const handleConfirm = () => {
    if (amountNum <= 0) { setAmountError('Số tiền phải lớn hơn 0'); return; }
    if (!walletId) { Alert.alert('', S.noWallet); return; }
    createMutation.mutate(
      { walletId, categoryId, amount: amountNum, type: entryType, description: merchant.trim() || null, merchant: merchant.trim() || null, transactionDate: dateIso, aiSuggestedCategoryId: categoryId, aiOverridden: false, entryMethod: 'sms_paste' },
      {
        onSuccess: () => Alert.alert('', S.saveOk, [{ text: 'OK', onPress: () => router.back() }]),
      ...
      <SafeAreaView style={styles.container} edges={['top']}>
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView contentContainerStyle={styles.reviewContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
            ...
            {/* Amount */}
            <View style={[styles.fieldCard, amountUncertain && styles.fieldCardUncertain]}>
              <Text style={styles.fieldLabel}>{S.fieldAmount}</Text>
              <RNTextInput
                value={amountRaw}
                onChangeText={(t) => { setAmountRaw(t.replace(/\D/g, '')); setAmountError(undefined); setAmountUncertain(false); }}
                keyboardType="numeric"
                placeholder="0"
                placeholderTextColor={COLORS.outlineVariant}
                style={styles.fieldInput}
              />
              {amountNum > 0 && <Text style={styles.amountPreview}>{formatVND(amountNum)}</Text>}
              {amountError && <Text style={styles.fieldError}>{amountError}</Text>}
            </View>

            {/* Merchant */}
            <View style={[styles.fieldCard, merchantUncertain && styles.fieldCardUncertain]}>
              <Text style={styles.fieldLabel}>{S.fieldMerchant}</Text>
              <RNTextInput
                value={merchant}
                onChangeText={(t) => { setMerchant(t); setMerchantUncertain(false); }}
            ...
            {/* Category */}
            <TouchableOpacity activeOpacity={0.7} style={[styles.fieldCard, categoryUncertain && styles.fieldCardUncertain]} onPress={() => setShowCategoryModal(true)}>
              <Text style={styles.fieldLabel}>{S.fieldCategory}</Text>
              <View style={styles.fieldRowValue}>
                {selectedCategory
                  ? <><View style={[styles.dot, { backgroundColor: selectedCategory.color }]} /><Text style={styles.fieldValueText}>{selectedCategory.nameVi}</Text></>
            ...
            {/* Wallet */}
            <TouchableOpacity activeOpacity={0.7} style={styles.fieldCard} onPress={() => setShowWalletModal(true)}>
              <Text style={styles.fieldLabel}>{S.fieldWallet}</Text>
              <View style={styles.fieldRowValue}>
                <Text style={styles.fieldValueText}>{selectedWallet?.name ?? ''}</Text>
                <MaterialIcon name="chevron_right" size={20} color={COLORS.outlineVariant} />
              </View>
            </TouchableOpacity>
            ...
            <View style={styles.reviewActions}>
              <TouchableOpacity activeOpacity={0.7} style={styles.rePasteBtn} onPress={() => setPhase('paste')}>
                <Text style={styles.rePasteBtnText}>{S.rePasteBtn}</Text>
              </TouchableOpacity>
              <TouchableOpacity activeOpacity={0.7} style={[styles.confirmBtn, createMutation.isPending && styles.disabled]} onPress={handleConfirm} disabled={createMutation.isPending}>
                <Text style={styles.confirmBtnText}>{S.confirmBtn}</Text>
                <MaterialIcon name="arrow_forward" size={18} color={COLORS.onPrimary} />
              </TouchableOpacity>
            </View>
          </ScrollView>
        </KeyboardAvoidingView>
        ...
        {/* Category modal */}
        <PickerModal visible={showCategoryModal} title={S.sheetCategory} onClose={() => setShowCategoryModal(false)}>
          <FlatList data={[...CATEGORIES]} keyExtractor={(i) => i.id} showsVerticalScrollIndicator={false}
            renderItem={({ item }) => (
              <TouchableOpacity style={[styles.sheetRow, categoryId === item.id && styles.sheetRowSelected]} onPress={() => { setCategoryId(item.id); setCategoryUncertain(false); setShowCategoryModal(false); }} activeOpacity={0.7}>
                <View style={[styles.sheetDot, { backgroundColor: item.color }]} />
                <Text style={styles.sheetRowText}>{item.nameVi}</Text>
                {categoryId === item.id && <MaterialIcon name="check" size={18} color={COLORS.primary} />}
              </TouchableOpacity>
            )}
          />
        </PickerModal>
        ...
        {/* Wallet modal */}
        <PickerModal visible={showWalletModal} title={S.sheetWallet} onClose={() => setShowWalletModal(false)}>
          <FlatList data={wallets} keyExtractor={(i) => i.id} showsVerticalScrollIndicator={false}
            renderItem={({ item }) => (
              <TouchableOpacity style={[styles.sheetRow, walletId === item.id && styles.sheetRowSelected]} onPress={() => { setWalletId(item.id); setShowWalletModal(false); }} activeOpacity={0.7}>
                ...
                {walletId === item.id && <MaterialIcon name="check" size={18} color={COLORS.primary} />}
              </TouchableOpacity>
            )}
          />
        </PickerModal>
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      <Header title={S.title} onBack={() => router.back()} rightLabel="Tiếp tục" onRightPress={smsText.trim().length >= 10 ? handleExtract : undefined} />

      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        <ScrollView contentContainerStyle={styles.pasteContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.textareaWrap}>
            ...
            <View style={styles.textareaActions}>
              {smsText.length > 0 && (
                <TouchableOpacity activeOpacity={0.7} style={styles.textareaActionBtn} onPress={() => setSmsText('')}>
                  <MaterialIcon name="close" size={16} color={COLORS.onSurface} />
                </TouchableOpacity>
              )}
              <TouchableOpacity activeOpacity={0.7} style={styles.textareaActionBtnPrimary}>
                <MaterialIcon name="content_paste" size={16} color={COLORS.primary} />
              </TouchableOpacity>
            </View>
          </View>
          ...
          <View style={styles.guideCard}>
            <View style={styles.guideHeader}>
              <MaterialIcon name="lightbulb" size={18} color={COLORS.secondary} />
              <Text style={styles.guideTitle}>{S.guide}</Text>
            </View>
            {S.guideSteps.map((step, i) => (
              <Text key={i} style={styles.guideStep}>{`${i + 1}. ${step}`}</Text>
            ))}
          </View>
        </ScrollView>
      </KeyboardAvoidingView>
      ...
      <View style={styles.bottomArea}>
        ...
        <TouchableOpacity
          activeOpacity={0.7}
          style={[styles.processBtn, smsText.trim().length < 10 && styles.disabled]}
...
function Header({
  ...
  return (
    <View style={styles.header}>
      <TouchableOpacity activeOpacity={0.7} style={styles.headerBtn} onPress={onBack}>
        <MaterialIcon name="arrow_back_ios" size={20} color={COLORS.primary} />
      </TouchableOpacity>
      <Text style={styles.headerTitle}>{title}</Text>
      {rightLabel ? (
        <TouchableOpacity activeOpacity={0.7} style={styles.headerBtn} onPress={onRightPress} disabled={!onRightPress}>
          <Text style={[styles.headerRight, !onRightPress && styles.disabled]}>{rightLabel}</Text>
        </TouchableOpacity>
      ) : (
...
const styles = StyleSheet.create({
  ...
  headerRight: { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.primary },
  ...
  instructionTitle: { fontSize: FONT_SIZE.base, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onSurface },
  instructionBody: { fontSize: FONT_SIZE.sm, color: COLORS.onSurfaceVariant, lineHeight: 20 },
  ...
  guideTitle: { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.secondary },
  guideStep: { fontSize: FONT_SIZE.sm, color: COLORS.onSurfaceVariant, lineHeight: 20 },
  ...
  processBtnText: { fontSize: FONT_SIZE.base, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onPrimary },
  ...
  fieldValueText: { fontSize: FONT_SIZE.base, fontWeight: FONT_WEIGHT.medium, color: COLORS.onSurface },
  fieldPlaceholder: { fontSize: FONT_SIZE.base, color: COLORS.outlineVariant },
  fieldRowValue: { flexDirection: 'row', alignItems: 'center', gap: SPACING[2] },
  fieldError: { fontSize: FONT_SIZE.xs, color: COLORS.error },
  amountPreview: { fontSize: FONT_SIZE.xl, fontWeight: FONT_WEIGHT.bold, color: COLORS.primary },
  dot: { width: 10, height: 10, borderRadius: BORDER_RADIUS.full },
  uncertainBadge: { fontSize: FONT_SIZE.xl, fontWeight: FONT_WEIGHT.bold, color: COLORS.warning, marginLeft: SPACING[1] },
  ...
  rePasteBtnText: { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onSurfaceVariant },
  confirmBtn: {
    flex: 2,
    height: 52,
    borderRadius: BORDER_RADIUS.lg,
  ...
  confirmBtnText: { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.bold, color: COLORS.onPrimary },
  ...
  sheetDot: { width: 14, height: 14, borderRadius: BORDER_RADIUS.full, flexShrink: 0 },
