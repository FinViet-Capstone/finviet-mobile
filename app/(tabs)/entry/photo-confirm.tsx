...
function ReviewRow({
  ...
  const cat = row.categoryId ? CATEGORIES.find((c) => c.id === row.categoryId) ?? null : null;
  ...
  return (
    ...
    >
      ...
      <View style={styles.reviewThumbWrap}>
        <Image source={{ uri: row.uri }} style={styles.reviewThumb} resizeMode="cover" />
      ...
      <View style={styles.reviewContent}>
        <View style={styles.reviewTopRow}>
          <Text style={styles.reviewImageLabel}>{S.imageOf(index + 1, total)}</Text>
          <View style={styles.reviewBadges}>
            {uncertain && (
              <View style={styles.uncertainBadge}>
                <MaterialIcon name="warning" size={12} color={COLORS.secondary} />
        ...
        {isFailed ? (
          ...
          <>
            ...
            <View style={styles.reviewField}>
              <Text style={[styles.reviewFieldLabel, row.amountUncertain && styles.uncertainLabel]}>
                {S.amountLabel}
              </Text>
              <Text style={[styles.reviewFieldValue, row.amountUncertain && styles.uncertainValue]}>
            ...
            {/* Merchant */}
            <View style={styles.reviewField}>
              <Text style={[styles.reviewFieldLabel, row.merchantUncertain && styles.uncertainLabel]}>
                {S.merchantLabel}
              </Text>
              <Text style={[styles.reviewFieldValue, row.merchantUncertain && styles.uncertainValue]} numberOfLines={1}>
                {row.merchant || '—'}
              </Text>
            </View>

            {/* Category — tappable */}
            <TouchableOpacity activeOpacity={0.7} style={styles.reviewField} onPress={onEditCategory}>
              <Text style={[styles.reviewFieldLabel, row.categoryUncertain && styles.uncertainLabel]}>
                {S.categoryLabel}
              </Text>
              <View style={styles.reviewCategoryRow}>
                {cat ? (
                  <>
                    ...
                    <Text style={[styles.reviewFieldValue, row.categoryUncertain && styles.uncertainValue]}>
                      {cat.nameVi}
                    </Text>
                  </>
                ) : (
            ...
            <View style={styles.reviewField}>
              ...
              <Text style={styles.reviewFieldValue}>{formatDate(row.dateIso)}</Text>
            </View>
          </>
        )}
      </View>
...
export default function PhotoConfirmScreen() {
  ...
  const { uris: rawUris, date: dateParam } = useLocalSearchParams<{ uris?: string; date?: string }>();
  const extract = useExtractFromPhoto();
  const createMutation = useCreateTransaction();
  const { data: walletsData } = useWallets();

  const uris: string[] = (() => {
    try { return JSON.parse(rawUris ?? '[]'); } catch { return []; }
  })();
  ...
  useEffect(() => {
    uris.forEach((uri, idx) => {
      extract.mutate(uri, {
        onSuccess: (result) => {
          setRows((prev) => {
            const updated = prev.map((r, i) => i !== idx ? r : {
              ...r,
              ...
              amountUncertain: result.confidence.amount < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD,
              merchantUncertain: result.confidence.merchant < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD,
              categoryUncertain: result.confidence.categoryId < PHOTO_EXTRACTION_CONFIDENCE_THRESHOLD,
            });
            // Mark duplicates: same amount + merchant + date
            const done = updated.filter((r) => r.status === 'done');
            return updated.map((r, i) => {
              if (r.status !== 'done') return r;
              const isDuplicate = done.some((other, oi) =>
                oi !== i && other.amount === r.amount && other.merchant === r.merchant && other.dateIso === r.dateIso
              );
        ...
        onError: () => {
          setRows((prev) => prev.map((r, i) => i === idx ? { ...r, status: 'failed' as const, selected: false } : r));
        },
      });
    });
  // run once on mount
  ...
  const handleCategorySelect = useCallback((categoryId: string) => {
    setRows((prev) => prev.map((r, i) => i === editingIdx ? { ...r, categoryId, categoryUncertain: false } : r));
    setEditingIdx(null);
  }, [editingIdx]);
  ...
  const handleConfirmAll = useCallback(async () => {
    ...
    if (!primary) { Alert.alert(S.noWallet, S.noWalletMsg); return; }
    ...
    try {
      ...
      Alert.alert('', S.savedMsg(toSave.length), [{ text: 'OK', onPress: () => router.back() }]);
    } catch {
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <View style={styles.bottomBar}>
        <TouchableOpacity activeOpacity={0.7} style={styles.retakeBtn} onPress={() => router.back()}>
      ...
      <DraggableSheet visible={editingIdx !== null} onClose={() => setEditingIdx(null)}>
        <View style={styles.catSheetContent}>
          ...
          <FlatList
            ...
            showsVerticalScrollIndicator={false}
            renderItem={({ item }) => {
              const selected = editingIdx !== null && rows[editingIdx]?.categoryId === item.id;
              return (
                <TouchableOpacity
                  activeOpacity={0.7}
...
const styles = StyleSheet.create({
  ...
  headerTitle: { flex: 1, textAlign: 'center', fontSize: FONT_SIZE.lg, fontWeight: FONT_WEIGHT.bold, color: COLORS.onSurface },

  listContent: { padding: SPACING[4], gap: SPACING[3], paddingBottom: SPACING[4] },
  ...
  reviewCheckOverlay: { position: 'absolute', top: 4, right: 4, backgroundColor: `${COLORS.background}CC`, borderRadius: BORDER_RADIUS.full },

  reviewContent: { flex: 1, gap: SPACING[2] },
  reviewTopRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  reviewImageLabel: { fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onSurfaceVariant },
  reviewBadges: { flexDirection: 'row', gap: SPACING[1] },

  uncertainBadge: { flexDirection: 'row', alignItems: 'center', gap: 3, backgroundColor: `${COLORS.secondary}20`, paddingHorizontal: SPACING[2], paddingVertical: 2, borderRadius: BORDER_RADIUS.full },
  uncertainBadgeText: { fontSize: 10, color: COLORS.secondary, fontWeight: FONT_WEIGHT.semibold },
  dupBadge: { backgroundColor: `${COLORS.tertiary}20`, paddingHorizontal: SPACING[2], paddingVertical: 2, borderRadius: BORDER_RADIUS.full },
  dupBadgeText: { fontSize: 10, color: COLORS.tertiary, fontWeight: FONT_WEIGHT.semibold },

  reviewField: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', minHeight: 20 },
  reviewFieldLabel: { fontSize: FONT_SIZE.xs, color: COLORS.onSurfaceVariant, flex: 1 },
  reviewFieldValue: { fontSize: FONT_SIZE.xs, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onSurface, flex: 2, textAlign: 'right' },
  ...
  retakeBtn: {
    flex: 1, height: 52, borderRadius: BORDER_RADIUS.lg,
    borderWidth: 1, borderColor: COLORS.outlineVariant,
    alignItems: 'center', justifyContent: 'center',
  },
  retakeText: { fontSize: FONT_SIZE.sm, fontWeight: FONT_WEIGHT.semibold, color: COLORS.onSurfaceVariant },
  confirmBtn: {
    flex: 2, height: 52, borderRadius: BORDER_RADIUS.lg,
